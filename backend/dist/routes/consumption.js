...
router.get('/', auth_1.authenticateToken, async (req, res, next) => {
    try {
        ...
        const consumptions = await Consumption_1.default.find(query).sort({ date: -1, shift: 1 });
...
router.get('/:id', auth_1.authenticateToken, async (req, res, next) => {
    try {
        const consumption = await Consumption_1.default.findById(req.params.id);
        if (!consumption) {
...
router.post('/', auth_1.authenticateToken, [
    (0, express_validator_1.body)('date').notEmpty().withMessage('Datum je obavezan'),
    (0, express_validator_1.body)('shift').isIn(['first', 'second', 'third']).withMessage('Smena mora biti first, second ili third'),
    (0, express_validator_1.body)('employeeName').notEmpty().withMessage('Ime zaposlenog je obavezno'),
    (0, express_validator_1.body)('resinId').notEmpty().withMessage('ID sarze je obavezan'),
    (0, express_validator_1.body)('usageCount').isNumeric().withMessage('Broj veziva mora biti broj')
], async (req, res, next) => {
    try {
        ...
        // Get resin details
        const resin = await Resin_1.default.findById(resinId);
        if (!resin) {
            return res.status(400).json({ message: 'Sarza nije pronađena' });
        }
        // Get material details
        const material = await Material_1.default.findById(resin.materialId);
        if (!material) {
        ...
        const inventory = await Inventory_1.default.findOne({ materialId: material._id });
        if (!inventory) {
        ...
        const consumption = new Consumption_1.default({
            date,
            shift,
            employeeName,
            resinId,
...
router.put('/:id', auth_1.authenticateToken, [
    (0, express_validator_1.body)('date').optional().notEmpty().withMessage('Datum ne sme biti prazan'),
    (0, express_validator_1.body)('shift').optional().isIn(['first', 'second', 'third']).withMessage('Smena mora biti first, second ili third'),
    (0, express_validator_1.body)('employeeName').optional().notEmpty().withMessage('Ime zaposlenog ne sme biti prazno'),
    (0, express_validator_1.body)('resinId').optional().notEmpty().withMessage('ID sarze je obavezan'),
    (0, express_validator_1.body)('usageCount').optional().isNumeric().withMessage('Broj veziva mora biti broj')
], async (req, res, next) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        // Update fields
        if (req.body.date)
            consumption.date = req.body.date;
        if (req.body.shift)
            consumption.shift = req.body.shift;
        if (req.body.employeeName)
            consumption.employeeName = req.body.employeeName;
        if (req.body.usageCount !== undefined)
            consumption.usageCount = req.body.usageCount;
        if (req.body.resinId) {
            const resin = await Resin_1.default.findById(req.body.resinId);
            if (!resin) {
                return res.status(400).json({ message: 'Sarza nije pronađena' });
            }
            consumption.resinId = resin._id.toString();
        ...
        const inventory = await Inventory_1.default.findOne({ materialId: consumption.materialId });
        if (!inventory) {
            return res.status(400).json({
...
router.delete('/:id', auth_1.authenticateToken, async (req, res, next) => {
    try {
        ...
        await Consumption_1.default.findByIdAndDelete(req.params.id);
